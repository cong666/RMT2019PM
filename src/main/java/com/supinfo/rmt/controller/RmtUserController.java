...
public class RmtUserController implements Serializable {
    ...
    public void doLogin() {
        ...
        if(user.getId()!=null) {
            FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put("login", user);
            try {
                FacesContext.getCurrentInstance().getExternalContext().redirect("authentication");
